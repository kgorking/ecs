...
TEST_CASE("entity_range ", "[entity]") {
	...
	SECTION("intersection tests") {
		SECTION("Empty range A") {
			...
			auto const result =
				ecs::detail::intersect_ranges_iter<Iter, Iter>({vec_a.begin(), vec_a.end()}, {vec_b.begin(), vec_b.end()});
		...
		SECTION("Empty range B") {
			...
			auto const result = ecs::detail::intersect_ranges_iter<Iter, Iter>({vec_a.begin(), vec_a.end()}, {vec_b.begin(), vec_b.end()});
